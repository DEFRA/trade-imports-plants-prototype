...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag trade-imports-plants-prototype`
...
```bash
docker run -p 3000:3000 trade-imports-plants-prototype:development
```
...
```bash
docker run -p 3000:3000 trade-imports-plants-prototype
```
